  ...
  return (
    <div className="overflow-auto border rounded-lg" style={{ maxHeight: '80vh' }}>
      <svg width={totalW} height={totalH} style={{ minWidth: '100%' }}>
          ...
          return (
            <g key={a.id}>
              ...
              <text x={6} y={y + 17} fontSize={11} fill={a.isCritical ? '#991b1b' : '#334155'}
                fontWeight={a.isCritical ? 600 : 400}>{a.wbsCode}</text>
              <text x={52} y={y + 17} fontSize={11} fill="#475569">
                {a.name.length > 22 ? a.name.slice(0, 21) + '…' : a.name}
              </text>
              {a.isMilestone ? (
                <polygon points={`${x0 + 8},${y + ROW_H / 2} ${x0 + 18},${y + 4} ${x0 + 28},${y + ROW_H / 2} ${x0 + 18},${y + ROW_H - 4}`} fill={color} />
              ) : (
                <>
                  ...
                  {w > 50 && <text x={x0 + 4} y={y + 17} fontSize={9} fill="#fff" fontWeight={600}>{pct}%</text>}
                  {w > 70 && <text x={x0 + w / 2} y={y + 17} fontSize={8} fill="#fff" textAnchor="middle" opacity={0.7}>{a.wbsCode}</text>}
                </>
              )}
            </g>
          )
        ...
        {todayInRange && (
          <>
            <line x1={LEFT_W + todayX * DAY_W} y1={0} x2={LEFT_W + todayX * DAY_W} y2={totalH}
              stroke="#ef4444" strokeWidth={2} />
            <rect x={LEFT_W + todayX * DAY_W - 22} y={0} width={44} height={HEADER_H} fill="#ef4444" rx={2} />
